...
class RedBlackTree {
    ...
    static <A,B> AOption<B> get(Tree<A,B> tree, A x, Comparator<A> ordering) {
        ...
        else return AOption.some(tree.value);
    }
    ...
    static <A> int countInRange(Tree<A,?> tree, AOption<A> from, AOption<A> to, Comparator<A> ordering) {
        ...
        if (from.nonEmpty() && ordering.compare(tree.key, from.get()) < 0) return countInRange(tree.right, from, to, ordering);

        // if node is greater than or equal to the upper bound, try the tree on the left, it might be in range
        if (to.nonEmpty() && ordering.compare(tree.key, to.get()) >= 0) return countInRange(tree.left, from, to, ordering);
    ...
    static<A,B> Tree<A,B> rangeImpl(Tree<A,B> tree, AOption<A> from, AOption<A> until, Comparator<A> ordering) {
        ...
        if (from.nonEmpty() && until.nonEmpty()) return range(tree, from.get(), until.get(), ordering);
        if (from.nonEmpty()) return from(tree, from.get(), ordering);
        else return until(tree, until.get(), ordering);
    }
    ...
    static <A,B> Tree<A,B> until(Tree<A,B> tree, A to, Comparator<A> ordering) {
        return blacken(doUntil(tree, to, ordering));
    }
    ...
    static <A,B> Tree<A,B> smallest(Tree<A,B> tree, Comparator<A> ordering) {
        if (tree == null) throw new NoSuchElementException("empty map");
        Tree<A,B> result = tree;
        while (result.left != null) result = result.left;
        return result;
    }
    static <A,B> Tree<A,B> greatest(Tree<A,B> tree, Comparator<A> ordering) {
        if (tree == null) throw new NoSuchElementException("empty map");
        Tree<A,B> result = tree;
    ...
    static <A,B> void foreach(Tree<A,B> tree, BiConsumer<A,B> f) { if (tree != null) _foreach(tree, f); }
    ...
    static <A> void foreachKey(Tree<A,?> tree, Consumer<A> f) { if (tree != null) _foreachKey(tree,f); }
    ...
    static <A,B> Iterator<Map.Entry<A,B>> iterator(Tree<A,B> tree, AOption<A> start, Comparator<A> ordering) { return new EntriesIterator<>(tree, start, ordering); }
    static <A> Iterator<A> keysIterator(Tree<A,?> tree, AOption<A> start, Comparator<A> ordering) { return new KeysIterator<>(tree, start, ordering); }
    static <A,B> Iterator<B> valuesIterator(Tree<A,B> tree, AOption<A> start, Comparator<A> ordering) { return new ValuesIterator<>(tree, start, ordering); }
    ...
    static boolean isBlack(Tree<?,?> tree) { return tree != null && isBlackTree(tree); }
    private static boolean isRedTree(Tree<?,?> tree) { return tree instanceof RedTree; } //TODO polymorphic 'isRed()'?
    private static boolean isBlackTree(Tree<?,?> tree) { return tree instanceof BlackTree; } //TODO polymorphic 'isBlack()'?

    private static <A,B> Tree<A,B> blacken(Tree<A,B> t) { if (t == null) return null; else return t.black(); }

    private static <A,B> Tree<A,B> mkTree(boolean isBlack, A k, B v, Tree<A,B> l, Tree<A,B> r) {
        if (isBlack) return new BlackTree<>(k, v, l, r); else return new RedTree<>(k, v, l, r);
    }
    ...
    private static <A,B> Tree<A,B> upd(Tree<A,B> tree, A k, B v, boolean overwrite, Comparator<A> ordering) { //TODO overwrite?!
    ...
    private static <A,B> Tree<A,B> del_append(Tree<A,B> tl, Tree<A,B> tr) {
        ...
        if (isBlackTree(tl) && isBlackTree(tr)) {
            ...
            return del_balLeft(tl.key, tl.value, tl.left, new BlackTree<>(tr.key, tr.value, bc, tr.right));
        }
    ...
    private static <A,B> Tree<A,B> doDrop(Tree<A,B> tree, int n) {
        ...
        final int count = count(tree.left); //TODO optimization: reuse this for 'count()' in the line above
    ...
    private static <A,B> CompareDepthResult<A,B> cd_unzipBoth(Tree<A,B> left, Tree<A,B> right, NList<Tree<A,B>> leftZipper, NList<Tree<A,B>> rightZipper, int smallerDepth) {
        ...
        if (left == null && isBlackTree(right)) {
            ...
            return new CompareDepthResult<A,B>(cd_unzip(rightZipper.prepend(right), leftMost), false, leftMost, smallerDepth);
        }
    ...
    private static final class NList<A> {
        ...
        NList (A head, RedBlackTree.NList<A> tail) {
            this.head = head;
            this.tail = tail;
        }
    ...
    static abstract class Tree<A,B> {
        ...
        int count() {
            return 1 + RedBlackTree.count(left) + RedBlackTree.count(right);
        }
    ...
    private static abstract class TreeIterator<A,B,R> implements Iterator<R> {
        ...
        TreeIterator (Tree<A, B> root, AOption<A> start, Comparator<A> ordering) {
            ...
            lookahead = start.map(this::startFrom).orElseGet(() -> findLeftMostOrPopOnEmpty(root));
            ...
            if (root == null) stackOfNexts = null;
                 ...
                 */
                final int maximumHeight = 2 * (32 - Integer.numberOfLeadingZeros(root.count() + 2 - 1)) - 2; //TODO verify that this actually helps - count() does have its cost...
